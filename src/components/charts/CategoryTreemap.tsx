      ...
      <div className="h-72 min-h-[288px] min-w-0">
        <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
          <Treemap data={data} dataKey="size" aspectRatio={4 / 3} content={<CustomContent x={0} y={0} width={0} height={0} />}>
            <Tooltip contentStyle={{ backgroundColor: "#18181b", border: "1px solid #3f3f46", borderRadius: "8px", fontSize: "12px" }} formatter={(value) => [formatCurrency(Number(value), true), "Volume"]} labelStyle={{ color: "var(--pm-fg-subtle)" }} />
          </Treemap>
        </ResponsiveContainer>
      </div>
    </div>
