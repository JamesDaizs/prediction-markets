        ...
        <div className="h-48 w-48 min-h-[192px] min-w-[192px]">
          <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
            <PieChart>
              ...
              <Tooltip contentStyle={{ backgroundColor: "#18181b", border: "1px solid #3f3f46", borderRadius: "8px", fontSize: "12px" }} formatter={(value) => [metric === "volume" ? formatCurrency(Number(value), true) : formatNumber(Number(value)), metric === "volume" ? "Volume" : "Traders"]} labelStyle={{ color: "var(--pm-fg-subtle)" }} />
            </PieChart>
          </ResponsiveContainer>
        </div>
        <div className="flex flex-col gap-2">
          {data.map((item, i) => (
            <div key={item.segment} className="flex items-center gap-2 text-sm">
              ...
              <span className="text-pm-fg-subtle">{item.segment}</span>
              <span className="text-pm-fg-muted">({formatNumber(item.count)} traders, {formatCurrency(item.volume, true)})</span>
            </div>
          ))}
        </div>
      </div>
